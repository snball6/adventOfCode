describe("day2", () => {
    ...
    describe("Part 1", () => {
        ...
        it("Should split games into objects - Sample", () => {
            ...
            expect(parseGame(sample[0])).toEqual(game1Object);
        });

        it("Should determine if game possible - Sample", () => {

            expect(isPossible(parseGame(sample[0]))).toBe(true);
            expect(isPossible(parseGame(sample[1]))).toBe(true);
            expect(isPossible(parseGame(sample[4]))).toBe(true);

            expect(isPossible(parseGame(sample[2]))).toBe(false);
            expect(isPossible(parseGame(sample[3]))).toBe(false);
        });


        it("Should sum possible games - Sample", () => {
            expect(sumPossible(sample)).toBe(8);
        });

        it("Should sum possible games - Actual", () => {
            expect(sumPossible(actual)).toBe(3059);
        });
    });
